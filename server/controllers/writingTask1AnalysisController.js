...
function normalizeVisualAsset(value) {
  ...
  const url = normalizeText(value?.url, "", 1200);
...
async function resolveTask1VisualContext(visualAsset) {
  const normalized = normalizeVisualAsset(visualAsset);
  if (!normalized.imageId) {
    return {
      visualAsset: normalized,
      visualImageDataUrl: "",
      visualMimeType: "",
    };
  }

  const visual = await WritingTask1Visual.findById(normalized.imageId)
    .select({ imageData: 1, mimeType: 1 })
    .lean();
  ...
  const visualMimeType = normalizeText(visual?.mimeType, "image/png", 80) || "image/png";
  return {
    visualAsset: normalized,
    visualImageDataUrl: `data:${visualMimeType};base64,${imageBuffer.toString("base64")}`,
    visualMimeType,
  };
}
...
function toAnalysisResponse(doc) {
  return {
    ...
    taskType: String(doc?.taskType || "writing_task1"),
    ...
    visualAsset: {
      imageId: String(doc?.visualAsset?.imageId || ""),
      url: String(doc?.visualAsset?.url || ""),
    },
...
async function createWritingTask1Analysis(req, res) {
  const body = req.body || {};
  ...
  const visualContext = await resolveTask1VisualContext(body.visualAsset);

  const analysis = await WritingTask1Analysis.create({
    studentUserId,
    studentEmail,
    taskType: "writing_task1",
    taskRefId,
    ...
    questionMeta: Array.isArray(body.questionMeta)
      ...
      : [],
    visualAsset: visualContext.visualAsset,
    essayText,
    wordsCount,
    durationSeconds,
  ...
  try {
    const aiResult = await evaluateWritingEssay({
      ...
      visualContext: {
        imageId: visualContext.visualAsset.imageId,
        url: visualContext.visualAsset.url,
        mimeType: visualContext.visualMimeType,
        imageDataUrl: visualContext.visualImageDataUrl,
      },
      wordsCount,
      timeSpentSeconds,
    });
...
async function getWritingTask1AnalysisById(req, res) {
  const analysisId = String(req.params.analysisId || "").trim();
  const analysis = await WritingTask1Analysis.findById(analysisId);
  if (!analysis) {
