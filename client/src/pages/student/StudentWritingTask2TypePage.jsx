...
function getTaskTitle(item) {
  ...
  return `Writing Task 2${question ? `: ${question}` : ""}`;
}

function getTargetWordsLabel(item) {
  const words = Number(item?.instruction?.minWords || 250);
  return `around ${Number.isFinite(words) && words > 0 ? words : 250} words`;
}
          ...
          {orderedItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-base font-semibold text-slate-700">{getTaskTitle(item)}</p>
                    <p className="mt-1 truncate text-xs text-slate-500">
                      {toReadableLabel(item?.essayType)} | {toReadableLabel(item?.status)} | {getTargetWordsLabel(item)}
                    </p>
                  </div>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-base font-semibold text-slate-900">{getTaskTitle(item)}</p>
                  <p className="mt-1 truncate text-xs text-slate-500">
                    {toReadableLabel(item?.essayType)} | {toReadableLabel(item?.status)} | {getTargetWordsLabel(item)}
                  </p>
                </div>
